...
from fireants.utils.imageutils import _find_integrator_n, scaling_and_squaring
...
class StationaryVelocity(nn.Module, AbstractDeformation):
    ...
    def __init__(
        ...
        if init_scale > 1:
            spatial_dims = [
                max(int(s / init_scale), MIN_IMG_SIZE) for s in spatial_dims
            ]
        ...
        if smoothing_grad_sigma > 0:
            ...
            self.smoothing_grad_gaussians = None
        ...
        self.optimizer = getattr(torch.optim, optimizer)(
            [self.velocity_field], lr=optimizer_lr, **deepcopy(optimizer_params)
        )
    ...
    def attach_grad_hook(self, restriction_mask=None):
        ...
        if hasattr(self.velocity_field, "_backward_hooks") and self.velocity_field._backward_hooks is not None:
            self.velocity_field._backward_hooks.clear()

        if self.smoothing_grad_sigma > 0 and self.smoothing_grad_gaussians is not None:
            if restriction_mask is not None:
                # Use anisotropic smoothing that respects restrictions
                from fireants.utils.util import anisotropic_grad_smoothing_hook

                hook = partial(
                    anisotropic_grad_smoothing_hook,
                    gaussians=self.smoothing_grad_gaussians,
                    restriction_mask=restriction_mask,
                )
            else:
                # Use normal isotropic smoothing
                from fireants.utils.util import grad_smoothing_hook

                hook = partial(
                    grad_smoothing_hook, gaussians=self.smoothing_grad_gaussians
                )
            self.velocity_field.register_hook(hook)

    def enable_anisotropic_grad_smoothing(self, restriction_mask):
        """Enable anisotropic gradient smoothing that respects restrictions"""
        self.attach_grad_hook(restriction_mask=restriction_mask)
    ...
    def set_size(self, size):
        ...
        old_optimizer_state["param_groups"] = self.optimizer.state_dict()[
            "param_groups"
        ]
...
if __name__ == "__main__":
    img1 = Image.load_file(
        "/data/BRATS2021/training/BraTS2021_00598/BraTS2021_00598_t1.nii.gz"
    )
    img2 = Image.load_file(
        "/data/BRATS2021/training/BraTS2021_00597/BraTS2021_00597_t1.nii.gz"
    )
    fixed = BatchedImages(
        [
            img1,
