...
from fireants.registration.distributed.utils import (
    add_distributed_padding,
    crop_distributed_padding,
)
import logging
...
def _get_smoothing_wrapper(optimizer):
    ...
    """
    gp_group = (
        parallel_state.get_parallel_state().get_current_gp_group()
    ...
    if gp_size <= 1:
        ...
        def smoothing_wrapper_nodist(tensor, kernels, padding=0):
            ...
            tensor = (
                separable_filtering(
                    tensor.permute(*optimizer.permute_vtoimg).contiguous(), kernels
                )
                .permute(*optimizer.permute_imgtov)
                .contiguous()
            )
            return tensor
        ...
        def smoothing_wrapper_dist(tensor, kernels, padding=0):
            if padding > 0:
                tensor = add_distributed_padding(
                    tensor, padding, optimizer.dim_to_shard - 1
                )  # 2 will be added to dim_to_shard anyway
            tensor = (
                separable_filtering(
                    tensor.permute(*optimizer.permute_vtoimg).contiguous(), kernels
                )
            ...
            if padding > 0:
                tensor = crop_distributed_padding(
                    tensor, padding, optimizer.dim_to_shard - 1
                )
            return tensor
...
class WarpAdam:
    ...
    def __init__(
        ...
        self.reset = reset
        self.scaledown = (
            scaledown  # if true, the scale the gradient even if norm is below 1
        )
        self.max_displacement = (
            max_displacement  # max allowed displacement in voxels (None = unlimited)
        )
        # offload params
        self.device = warp.device
        self.adam_update_kernel = (
            adam_update_fused
            if self.device.type == "cuda"
            else adam_update_fused_baseline
        )
        self.offload = offload
        # warp grad params
        self.exp_avg = torch.zeros_like(
            warp, device=self.device if not self.offload else "cpu"
        )
        self.exp_avg_sq = torch.zeros_like(
            warp, device=self.device if not self.offload else "cpu"
        )
        self.permute_imgtov = (
            0,
        ...
        # init grid
        self.affine_init = (
            torch.eye(self.n_dims, self.n_dims + 1, device=warp.device, dtype=dtype)[
                None
            ]
        ...
        if self.smoothing_gaussians is not None:
            self.padding_smoothing = (
                [len(x) for x in self.smoothing_gaussians]
                if isinstance(self.smoothing_gaussians, (list, tuple))
                else [len(self.smoothing_gaussians) for _ in range(self.n_dims)]
            )
            self.padding_smoothing = (
                self.padding_smoothing[self.dim_to_shard] - 1
            ) // 2
        else:
        ...
        if USE_NO_GP:
            logger.warning(
                f"⚠️ Overriding GP with no GP (use this setting with caution)"
            )
            self.padding_smoothing = 0
        ...
        if any([s != size[i] for i, s in enumerate(self.exp_avg.shape[1:-1])]):
            ...
            if self.reset:
                ...
                self.exp_avg = torch.zeros(
                    warp_size, device=self.device if not self.offload else "cpu"
                )
                ...
                self.exp_avg = F.interpolate(
                    self.exp_avg.detach().permute(*self.permute_vtoimg),
                    size=size,
                    mode=mode,
                    align_corners=True,
                ).permute(*self.permute_imgtov)
                self.exp_avg_sq = F.interpolate(
                    self.exp_avg_sq.detach().permute(*self.permute_vtoimg),
                    size=size,
                    mode=mode,
                    align_corners=True,
                ).permute(*self.permute_imgtov)
    ...
    def augment_jacobian(self, u):
        ...
        if fireants_interpolator.use_ffo:
            ...
            logger.warning(
                "Using fused grid sampler, Jacobian is not computed, returning input"
            )
            return u
        else:
            jac = jacobian_fn(
    ...
    def step(self):
        ...
        if self.freeform:
            ...
            if self.smoothing_gaussians is not None:
                self.warp.data = self.smoothing_wrapper(
                    self.warp.data, self.smoothing_gaussians, self.padding_smoothing
                )
        else:
            # This is the diffeomorphic update
            # gradmax = self.eps + grad.reshape(grad.shape[0], -1).abs().max(1).values  # [B,]
            gradmax = (
                self.eps + grad.norm(p=2, dim=-1, keepdim=True).flatten(1).max(1).values
            )
            gradmax = gradmax.reshape(-1, *([1]) * (self.n_dims + 1))
            if (
                not self.scaledown
            ):  # if scaledown is "True", then we scale down even if the norm is below 1
            ...
            grad.add_(
                fireants_interpolator.warp_composer(
                    self.warp.data,
                    affine=self.affine_init,
                    v=grad,
                    grid=self.grid,
                    align_corners=True,
                )
