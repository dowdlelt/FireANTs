...
class SyNRegistration(AbstractRegistration, DeformableMixin):
    ...
    def get_warp_parameters(
        ...
        affine_map_init = (
            (torch.matmul(moving_p2t, torch.matmul(self.affine, fixed_t2p))[:, :-1])
            .contiguous()
            .to(self.dtype)
        )
        ...
        if (
            self._cached_rev_inv_warp is not None
            and self._cached_rev_inv_warp_shape == tuple(shape[2:])
        ):
        ...
        if self.smooth_warp_sigma > 0:
            warp_gaussian = [
                ...
                for s in (
                    torch.zeros(self.dims, device=fixed_arrays.device, dtype=self.dtype)
    ...
    def get_inverse_warp_parameters(
        ...
        affine_map_inv = (
            (torch.matmul(fixed_t2p, torch.matmul(affine_inv, moving_p2t))[:, :-1])
            .contiguous()
            .to(self.dtype)
        )
        ...
        if (
            self._cached_fwd_inv_warp is not None
            and self._cached_fwd_inv_warp_shape == tuple(shape[2:])
        ):
            fwd_inv_warp_field = self._cached_fwd_inv_warp
        else:
            from fireants.utils.warputils import compositive_warp_inverse

            fwd_inv_warp_field = compositive_warp_inverse(
                fixed_images,
                self.fwd_warp.get_warp(),
                displacement=True,
                restrict_deformation=restrict_deform,
                progress_bar=False,
            )
        ...
        if self.smooth_warp_sigma > 0:
            ...
            rev_warp_field = separable_filtering(
                rev_warp_field.permute(*self.rev_warp.permute_vtoimg), warp_gaussian
            ).permute(*self.rev_warp.permute_imgtov)
            fwd_inv_warp_field = separable_filtering(
                fwd_inv_warp_field.permute(*self.fwd_warp.permute_vtoimg), warp_gaussian
            ).permute(*self.fwd_warp.permute_imgtov)
    ...
    def optimize(self):
        ...
        init_grid = (
            ...
            .to(self.fixed_images.device, self.dtype)
            .unsqueeze(0)
        ...
        for scale, iters in zip(self.scales, self.iterations):
            ...
            for i in pbar:
                ...
                moved_image_warp = fireants_interpolator(
                    moving_image_blur,
                    affine=affine_map_init,
                ...
                # reverse warp
                fixed_image_warp = fireants_interpolator(
                    fixed_image_down,
                    affine=None,
                    grid=rev_warp_field.contiguous(),
                    mode="bilinear",
                    align_corners=True,
                    is_displacement=True,
                )
                ...
                if self.warp_reg is not None:
                    ...
                    moved_coords = fwd_warp_field + F.affine_grid(
                        affine_map_init, fixed_image_down.shape, align_corners=True
                    )
                    fixed_coords = rev_warp_field + F.affine_grid(
                ...
                if self.warp_penalty > 0:
                    loss = loss + self.warp_penalty * (
                        fwd_warp_field.pow(2).mean() + rev_warp_field.pow(2).mean()
                    )
                # backward
                loss.backward()
...
if __name__ == "__main__":
    ...
    for img_dtype in [torch.bfloat16, torch.float32]:
        ...
        img1 = Image.load_file(
            f"{path}/BRATS2021/training/BraTS2021_00598/BraTS2021_00598_t1.nii.gz",
            dtype=img_dtype,
        )
        img2 = Image.load_file(
            f"{path}/BRATS2021/training/BraTS2021_00597/BraTS2021_00597_t1.nii.gz",
            dtype=img_dtype,
        )
        fixed = BatchedImages(
            [
