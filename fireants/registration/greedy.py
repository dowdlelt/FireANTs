...
class GreedyRegistration(AbstractRegistration, DeformableMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.has_partial_restriction:
            ...
            self.warp.enable_anisotropic_grad_smoothing(self.restrict_deformation)
    ...
    def get_inverse_warped_coordinates(
        ...
        warp = warp + F.affine_grid(
            torch.eye(self.dims, self.dims + 1, device=warp.device)[None],
        ...
        if displacement:
            grid = F.affine_grid(
                torch.eye(self.dims, self.dims + 1, device=grid.device)[None],
                shape,
                align_corners=True,
            )
    ...
    def get_warped_coordinates(
        ...
        if displacement:
            init_grid = F.affine_grid(
                torch.eye(self.dims, self.dims + 1, device=moved_coords.device)[None],
                fixed_images.shape,
                align_corners=True,
            )
