...
def make_gaussian_kernel_alt(kernel_size: int) -> torch.Tensor:
    ...
    if kernel_size % 2 == 0:
        raise ValueError(
            f"kernel_size must be odd for Gaussian kernel, got {kernel_size}"
        )
    ...
    kernel = gaussian_1d(
        sigma=sigma, truncated=3.0, approx="sampled", normalize=False
    ) * (2.5066282 * sigma)
...
def _separable_filtering_conv(
    ...
    for d in range(spatial_dims):
        ...
        if spatial_dims == 1:
            input_ = F.conv1d(
                input=padded_input, weight=_kernel, groups=num_channels, padding="same"
            )
        elif spatial_dims == 2:
            input_ = F.conv2d(
                input=padded_input, weight=_kernel, groups=num_channels, padding="same"
...
def separable_filtering(
    x: torch.Tensor,
    kernels: ItemOrList[torch.Tensor],
    mode: str = "zeros",
    ...
    if _kernels[0].numel() < 7 and not use_separable_override:
        ...
        if spatial_dims == 2:
            ...
            kernel = (
                _kernels[0].reshape(1, 1, -1, 1, 1)
                * _kernels[1].reshape(1, 1, 1, -1, 1)
                * _kernels[2].reshape(1, 1, 1, 1, -1)
            )
...
class LocalNormalizedCrossCorrelationLoss(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(kernel_size, int):
            ...
            if len(kernel_size) != self.ndim:
                raise ValueError(
                    f"kernel_size list must have {self.ndim} elements, got {len(kernel_size)}"
                )
        ...
        if self.intensity_weighting and self.intensity_weight_sigma > 0:
            self.intensity_weight_gaussians = [
                gaussian_1d(self.intensity_weight_sigma, truncated=2)
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # sum over kernel
        def cc_checkpoint_fn(
            target, pred, kernel, kernel_vol, aniso_kernels, checkpointing=False
        ):
            """
            ...
            kernels = (
                [k.to(pred) for k in aniso_kernels]
                if aniso_kernels
                else [kernel] * self.ndim
            )
            ...
            def sum_filter(target, kernels_t):
                t_sum = separable_filtering(
                    target,
                    kernels=kernels_t,
                    use_separable_override=self.use_separable_override,
                )
                return t_sum
            ...
            def cross_filter(tp_sum, p_sum, t_sum, kernel_vol):
                return (
                    tp_sum.to(pred) - p_sum * t_sum.to(pred) / kernel_vol
                )  # on pred device

            if checkpointing:
                cross = checkpoint(
                    cross_filter, tp_sum, p_sum, t_sum, kernel_vol, use_reentrant=False
                )
            else:
            ...
            def var_filter(t2sum, tsum, kernel_vol):
                return torch.max(
                    t2sum - tsum * tsum / kernel_vol,
                    torch.as_tensor(
                        self.smooth_dr, dtype=t2sum.dtype, device=t2sum.device
                    ),
                ).to(pred)

            if checkpointing:
                t_var = checkpoint(
                    var_filter, t2_sum, t_sum, kernel_vol, use_reentrant=False
                )
                p_var = checkpoint(
                    var_filter, p2_sum, p_sum, kernel_vol, use_reentrant=False
                )
            else:
            ...
            if self.unsigned:
                ...
                if checkpointing:
                    ncc = checkpoint(
                        ncc_filter, cross, t_var, p_var, use_reentrant=False
                    )
                else:
        ...
        ncc = cc_checkpoint_fn(
            ...
            self.kernel,
            self.kernel_vol,
            self.kernels if not self.is_isotropic else None,
        ...
        if self.intensity_weighting:
            ...
            if self.intensity_weight_gaussians is not None:
                ...
                target_smooth = separable_filtering(
                    target, self.intensity_weight_gaussians
                )
        ...
        if self.reduction == "sum":
            return torch.sum(
                weighted_ncc
            ).neg()  # sum over the batch, channel and spatial ndims
...
if __name__ == "__main__":
    ...
    for use_jit_version, separable_override in [
        (True, True),
        (True, False),
        (False, True),
        (False, False),
        ...
        loss = LocalNormalizedCrossCorrelationLoss(
            ...
            reduction="mean",
            use_separable_override=separable_override,
        ).cuda()
        if use_jit_version:
        ...
        b = time()
        print(
            f"Time for jit: {use_jit_version} separable: {separable_override} time: {b - a}s"
        )
        print(f"Total loss: {total}")
        mem = torch.cuda.max_memory_allocated() - mem
        print(
            f"Memory for jit: {use_jit_version} separable: {separable_override} memory: {mem / 1024 / 1024} MB\n"
        )
        print()
